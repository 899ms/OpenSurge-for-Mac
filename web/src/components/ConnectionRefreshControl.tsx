...
export function ConnectionRefreshControl({
    ...
    try {
      ...
      setFeedback({
        ...
        message: result.closed_connections > 0
          ? t('已关闭 {{count}} 个连接，等待客户端建立新连接。', { count: result.closed_connections })
          : t('当前没有需要刷新的连接。'),
      })
  ...
  return <div className="connection-refresh-control">
    <div className="connection-refresh-copy">
      <strong>{t('刷新连接')}</strong>
      <small>{t(disabled && disabledReason ? disabledReason : refreshExplanation)}</small>
    </div>
    <button className="connection-refresh-button" type="button" aria-label={t(ariaLabel)} disabled={disabled || busy} onClick={() => void run()}>{t(busy ? '正在刷新…' : '刷新连接')}</button>
    {feedback ? <small className={`connection-refresh-feedback ${feedback.tone}`} role={feedback.tone === 'error' ? 'alert' : 'status'}>{t(feedback.message)}</small> : null}
  </div>
}
